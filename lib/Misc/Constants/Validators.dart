...
mixin Validators {
  ...
  var loginPasswordValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (password, sink) {
      ...
      if (password.length < 3) {
        return sink.addError("Password can't be less than 3 characters");
      }
      if (password.length > 15) {
        return sink.addError("Password can't be more than 15 characters");
      } else {
        sink.add(password);
      }
    },
  ...
  var schoolValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (name, sink) {
      ...
      if (!RegExp(r"^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$")
          .hasMatch(name)) {
        return sink.addError("Invalid School Name");
      } else {
        sink.add(name);
      }
    },
  ...
  var passwordValidator = StreamTransformer<String, String>.fromHandlers(
    handleData: (password, sink) {
      ...
      if (!RegExp(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{3,15}$")
          .hasMatch(password)) {
        return sink.addError(
            "Password must contain one small,capital and a special character");
      } else {
        sink.add(password);
      }
    },
